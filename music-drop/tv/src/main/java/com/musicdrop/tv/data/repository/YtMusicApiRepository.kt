            ...
            for (i in 0 until data.length()) {
                ...
                if (resultType == "song" || resultType == "video") {
                    ...
                    if (vid.isNotBlank()) {
                        ...
                        if (!com.musicdrop.tv.data.youtube.YouTubeMusicRepository.isLongVideoOrJukebox(title, dur)) {
                            val artists = item.optJSONArray("artists")
                            val artistName = if (artists != null && artists.length() > 0) {
                                (0 until artists.length()).mapNotNull { idx ->
                                    artists.optJSONObject(idx)?.optString("name")
    ...
    suspend fun getArtist(channelId: String): YtArtistDetails? = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (songsList.size < 15 && name.isNotBlank()) {
                try {
                    val searchOutcome = com.musicdrop.tv.data.youtube.YouTubeSearchRepository.search("$name songs official", maxResults = 25)
                    if (searchOutcome is com.musicdrop.tv.data.youtube.YouTubeSearchOutcome.Success) {
